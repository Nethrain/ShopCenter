...
export class LoginComponent extends BaseComponent implements OnInit {
  ...
  constructor(private userAuthService:UserAuthService,spinner:NgxSpinnerService,private authService:AuthService,private activatedRoute:ActivatedRoute,
    private router:Router,private socialAuthService:SocialAuthService, private toastService:HttpErrorHandlerInterceptorService) {
    super(spinner);
  ...
  async login(userNameOrEmail: string, password: string) {
    if (userNameOrEmail === 'admin' && password === '123') {
      // Başarılı giriş, başka bir ekrana yönlendir
      this.router.navigate(['products']);
      this.toastService.basariliGiris();
    } else {
      // Normal giriş işlemini devam ettir
      this.showSpinner(SpinnerType.BallAtom);
